...
from scraper.providers.utils import format_measurements, format_nutrition
...
class HelloFreshScraper(BaseScraper):
    ...
    def ingredients(self) -> str:
        measurements = ["ounce", "tablespoon", "unit", "cup", "teaspoon"]
        ...
        in_your_pantry = [format_measurements(tag.text, measurements) for tag in list(div.children)[1].contents[0]]
        return "\n".join(in_the_box + in_your_pantry)

    @property
    def nutritional_info(self) -> str:
        measurements = ["Portion", "kJ", "kcal", "g", "mg"]
        parent_tag = self.soup.find("div", {"data-test-id": "recipeDetailFragment.nutrition-values"})
        div = list(parent_tag)[-1]
        return "\n".join([format_nutrition(line) for line in list(div.children)[0]])
    ...
    def prep_time(self) -> str:
        return self.soup.find("span", {"data-translation-id": "recipe-detail.cooking-time"}).next.next.text
